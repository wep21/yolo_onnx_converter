...
class WeightLoader(object):
    ...
    def _open_weights_file(self, weights_file_path):
        ...
        length_header = 5
        np.ndarray(
            shape=(length_header,), dtype="int32", buffer=weights_file.read(length_header * 4)
        )
        return weights_file
    ...
    def _load_one_param_type(self, conv_params, param_category, suffix):
        ...
        param_data = np.ndarray(
            shape=param_shape, dtype="float32", buffer=self.weights_file.read(param_size * 4)
        )
...
class GraphBuilderONNX(object):
    ...
    def __init__(self, output_tensors):
        ...
        self.epsilon_bn = 5e-4
        self.momentum_bn = 0.949
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        if self.input_tensor is None:
            ...
            if layer_type in node_creators.keys():
                major_node_output_name, major_node_output_channels = node_creators[layer_type](
                    layer_name, layer_dict
                )
                major_node_specs = MajorNodeSpecs(
                    major_node_output_name, major_node_output_channels
    ...
    def _make_conv_node(self, layer_name, layer_dict):
        ...
        if inputs[0] in self.route_dict.keys():
            inputs = [self.route_dict[inputs[0]].name]
        ...
        if layer_dict["activation"] == "leaky":
            ...
            logistic_node = helper.make_node(
                "Sigmoid", inputs=inputs, outputs=[layer_name_logistic], name=layer_name_logistic
            )
            ...
            softplus_node = helper.make_node(
                "Softplus", inputs=inputs, outputs=[layer_name_softplus], name=layer_name_softplus
            )
            ...
            tanh_node = helper.make_node(
                "Tanh", inputs=inputs_t, outputs=[layer_name_tanh], name=layer_name_tanh
            )
            ...
            mul_node = helper.make_node(
                "Mul", inputs=inputs, outputs=[layer_name_mish], name=layer_name_mish
            )
    ...
    def _make_route_node(self, layer_name, layer_dict):
        ...
        if len(route_node_indexes) == 1:
            ...
            if "groups" in layer_dict and "group_id" in layer_dict:
                inputs = []
                outputs = []
                outputs.append(layer_name + "_0")
                outputs.append(layer_name + "_1")
                route_node_specs = self._get_previous_node_specs(target_index=split_index)
                ...
                channels = int(route_node_specs.channels / 2)
                group_id = layer_dict["group_id"]
                return outputs[group_id], channels
            else:
                if split_index > 0:
                    split_index += 1
                self.route_dict[layer_name] = self._get_previous_node_specs(
                    target_index=split_index
                )
                channels = self.route_dict[layer_name].channels
        else:
            inputs = list()
            channels = 0
    ...
    def _make_maxpool_node(self, layer_name, layer_dict):
        ...
        inputs = [layer_name]
        layer_name_output = layer_name
        return layer_name_output, previous_channels


def process_mish_nodes(graph):
    softplus_nodes = [node for node in graph.nodes if node.op == "Softplus"]
    for node in softplus_nodes:
        mish_input = node.i().outputs[0]
        node.inputs = []
        mish_output = node.o().o().outputs[0]
        mish_name = node.o().o().name
        node.o().o().outputs = []
        node.o().o().inputs = []
        mish_node = gs.Node(
            "Mish_TRT",
...
def main():
    parser = argparse.ArgumentParser(
        description="Create onnx file from yolov4 config and weight file."
    )
    ...
    parser.add_argument(
        "--yolo_type",
        help="yolo type",
        choices=["yolov3", "yolov4", "yolov4-tiny"],
        default="yolov4",
        type=str,
    )
    parser.add_argument("--use_custom_plugin", help="use custom plugin", action="store_true")
    parser.add_argument(
        "--output_file_path", "-o", help="output file path", default="yolov4.onnx", type=str
    )
    ...
    # Simplify model
    yolo_model_def, check = simplify(yolo_model_def)
    assert check, "Simplified ONNX model could not be validated"

    # Perform a sanity check on the ONNX model definition:
    onnx.checker.check_model(yolo_model_def)

    if opt.yolo_type == "yolov4" and opt.use_custom_plugin:
        graph = gs.import_onnx(yolo_model_def)
        graph = process_mish_nodes(graph)
        graph.cleanup().toposort()
        yolo_model_def = gs.export_onnx(graph)
